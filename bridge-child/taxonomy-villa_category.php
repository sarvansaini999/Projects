...
$slug = get_query_var( 'term' );
 $term = get_term_by('slug', $slug, 'villa_category'); 
  $category_name = $slug; 
    
?>
<div class="title_outer title_without_animation" data-height="730">
	<div class="background-fixed title title_size_large  position_center  has_background" style="padding-top:100px;height:635px; background:url('http://www.hinolincolnshire.co.uk/wp-content/uploads/2015/04/Bali-Villas.jpg') !important;background-attachment: fixed !important;background-size: 1920px auto;background-repeat: no-repeat !important;background-position: center 0 !important;">
		<div class="image not_responsive"><img src="http://www.hinolincolnshire.co.uk/wp-content/uploads/2015/04/Bali-Villas.jpg"/></div>
...
<?php	
    ...
    $html .= "";
		...
		$cat_terms = wp_get_post_terms(get_the_ID(), 'category');
		...
		foreach ($terms as $term) {
			$html .= "villa_category_$term->term_id $term->slug ";
		}
		...
		$lowestPrice = intval(str_replace(',', '', $lowPrices));
		$html .="' data-cat='$lowestPrice'>";
		...
		if ($type == "" ) {
			...
			$html .= '<p class="thumb-text villa-thumb-text">'. $short_desc .'</p>';
		...
		if ($type == "" ) {
			...
			$html .= '<h5'.$title_tag.' class="portfolio_title "><a href="' . $portfolio_link . '" target="'.$target.'">' . get_the_title() . '</a></'.$title_tag.'></h5><p class="vinfo">'.$bedrooms;
			if($beachfront != ""){
			$html .= ' | '.$beachfront;
			}
			if($address != ""){
