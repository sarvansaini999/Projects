...
$categories = wp_get_object_terms( get_the_ID(), 'retreat_category');
 foreach($categories as $category){
	$term_name = $category->name;
 }
	if ($categories) {
		$args = array(
			'retreat_category' => $term_name, 
			'post_type' => 'retreats',
			'orderby' => 'rand',
			'order' => 'DESC',
			'posts_per_page' => 3,
...
$all_categories = wp_get_object_terms( get_the_ID(), 'retreat_category');

	if ($all_categories) {
		$map_args = array(
			'retreat_category' => $all_categories, 
			'post_type' => 'retreats',
			'orderby' => 'rand',
			'order' => 'DESC',
			'cache_results' => false,
		...
		<?php if( $map_query->have_posts() ) { 
			...
			while( $map_query->have_posts() ) {
				...
				echo '<script>var arrayFromPhp = ' . json_encode($locations) . ';//alert(arrayFromPhp);</script>';
		}
$post = $map_post;
wp_reset_query(); ?>