...
$slug = get_query_var( 'term' );
 $term = get_term_by('slug', $slug, 'yacht_category'); 
 $category_name = $slug; 
...
<div class="title_outer title_without_animation" data-height="735">
	...
	<div class="background-fixed title title_size_large  position_center  has_background" style="padding-top:100px;height:635px; background:url('http://www.hinolincolnshire.co.uk/wp-content/uploads/2015/04/Indonesian-Yacht-Charters.jpg') !important;background-attachment: fixed !important;background-size: 1920px auto;background-repeat: no-repeat !important;background-position: center 0 !important;">
		<div class="image not_responsive"><img src="http://www.hinolincolnshire.co.uk/wp-content/uploads/2015/04/Indonesian-Yacht-Charters.jpg"/></div>
		...
		if ($filter == "yes") {
			...
			if($type == 'masonry_with_space'){
				...
				foreach ($portfolio_categories as $yacht_category) {
					$html .= "<li class='filter' data-filter='.villa_category_$yacht_category->term_id'><span>$yacht_category->name</span>";
					$args = array(
				...
				foreach ($portfolio_categories as $yacht_category) {
					$html .= "<li class='filter' data-filter='villa_category_$yacht_category->term_id'><span". $filter_style .">$yacht_category->name</span>";
					$args = array(
						'child_of' => $yacht_category->term_id
					);
					$html .= '</li>';
