							...
							<div class="column_inner">
								<div class="contact_form">
									...
									<form id="contact-form" method="post" action="">
										...
										<div class="column1">
											<div class="column_inner">
												<input type="text" class="requiredField subject" name="subject" id="subject" value="" placeholder="<?php _e('Subject ', 'qode'); ?>" />
    ...
    $j('form#contact-form').submit(function(){
        ...
        $j('form#contact-form .requiredField').each(function() {
          ...
          if(!hasError){
			...
			var html = $j.ajax({
			type: "POST",
			url: "<?php echo QODE_ROOT; ?>-child/includes/ajax_mail.php",
			data: "recaptcha_challenge_field=" + challengeField + "&recaptcha_response_field=" + responseField + "&name=" + name + "&lastname=" + lastname + "&email=" + email + "&website=" + website + "&subject=" + subject + "&message=" + message,
			async: false
			}).responseText;
			...
			if (html.indexOf("success") >= 0) {
				...
				$j(".contact_detail").before("<div class='contact-success'><h3><?php _e('Thank you for contacting us <br/> we will be in touch with you very shortly.<br/> <br/> ', 'qode'); ?></h3></div>");
				$j(".contact_detail").hide();
